...
// bit 是从右往左的位置，num是数字， point是是否含小数点
void LED_Show(Uint8 Bit, Uint8 Num, Uint8 Point) {
	if (1 == Point)
		TM1638_WriteLED(2 * (8 - Bit), codetable[Num] + 0x80);
	else
		TM1638_WriteLED(2 * (8 - Bit), codetable[Num]);
}
void LED_showfloat(Uint8 bit,float num)
{
    Uint8 decade, perde, perhund;
    decade = ((int)num) % 10;
    perde = ((int)(num*10)) % 10;
    perhund = ((int)(num*100)) % 10;
    LED_Show(bit+2, decade, 1);
    LED_Show(bit+1, perde ,0);
    LED_Show(bit, perhund, 0);
}
